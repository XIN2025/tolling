...
func (c *HTTPClient) GetInvoice(ctx context.Context, id int) (*types.Invoice, error) {
	...
	if resp.StatusCode != http.StatusOK {
		// Read the response body to log the error message
		bodyBytes, _ := io.ReadAll(resp.Body) // Read the body for logging
		return nil, fmt.Errorf("the service responded with non-200 status code %d: %s", resp.StatusCode, string(bodyBytes))
	}
